...
export const MarkdownAnswer = ({ text, index, onOptionClick }: { text: string; index: RefIndex | null; onOptionClick?: (q: string) => void }) => {
  if (!index) {
        ...
        components={{
          ...
          a: ({ href, children }) => {
            if (href?.startsWith("?q=")) {
              const q = decodeURIComponent(href.slice(3).replace(/\+/g, " "));
              return (
                <button
                  type="button"
                  onClick={() => onOptionClick?.(q)}
                  className="my-1 inline-flex items-center gap-1.5 rounded-full bg-raised/80 border border-line px-3.5 py-1.5 text-[0.8rem] font-medium text-ink transition hover:bg-raised hover:text-brand hover:border-brand/40 shadow-sm active:scale-[0.98] text-left"
                >
