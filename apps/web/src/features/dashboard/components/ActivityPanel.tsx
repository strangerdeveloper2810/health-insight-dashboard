...
import { Panel, toneText } from "@/shared/ui/primitives";
              ...
              option={{
                ...
                tooltip: {
                  ...
                  formatter: (params: any) => {
                    const data = params[0];
                    return `Week of ${formatDayMonth(data.name)}<br/>${Number(data.value).toFixed(1)} km`;
                  },
                },
                xAxis: {
                  type: "category",
                  data: weekly.map((d) => d.date),
                  axisLabel: { formatter: (value: string) => formatDayMonth(value), color: palette.axisText, fontSize: 10 },
                  axisLine: { show: false },
                  axisTick: { show: false },
                },
                yAxis: {
                ...
                series: [
                  {
                    type: "bar",
                    data: weekly.map((d) => d.value),
                    itemStyle: { color: palette.alt, borderRadius: [3, 3, 0, 0] },
                    animation: false,
                  },
                ],
              }}
