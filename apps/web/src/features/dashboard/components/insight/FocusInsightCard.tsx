...
/**
 * The one card the reader is meant to act on — visually prioritized.
 */
export const FocusInsightCard = ({ insight }: { insight: Insight }) => {
  const { open, toggle } = useInsightDisclosure(insight.id);

  return (
    <div className="relative h-full flex flex-col overflow-hidden rounded-tile bg-surface border border-line px-5 py-5 sm:px-6 shadow-sm">
